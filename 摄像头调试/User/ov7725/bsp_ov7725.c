...
/**
	橙色识别函数
**/
uint8_t color_check(uint16_t tmp[], uint16_t H_lo, uint16_t H_hi) {
	...
	//if((h > 4400 && h < 4700) || (h > 4800 && h < 4900) || (h > 6800 && h < 7000) || (h > 7200 && h < 7300)) return 1;	
	if((h > 4400 && h < 4700) || (h > 4800 && h < 4900) || (h > 6800 && h < 7000)) return 1;
//if( h < H_lo || h > H_hi ) return 0;
	return 0;
}
...
/**
  ...
  * @retval 无
  */
uint16_t ImagDisp1(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height, uint16_t H_lo, uint16_t H_hi, uint16_t red_point_cnt)
{
	uint16_t i, j;
	uint16_t Camera_Data;
	uint16_t tmp[3];
	uint16_t tmpcnt = 0;
	//uint8_t batch[320];	//桶排序数组
	
	ILI9341_OpenWindow(sx,sy,width,height);
	ILI9341_Write_Cmd ( CMD_SetPixel );	
	
//	for(i = 0; i < width; i++) {//桶清零
//		batch[i] = 0;
//	}
	for(i = 0; i < height; i++)
	{
		for(j = 0; j < width; j++)
		{
			...
			if(i > 35 && i < 125 && color_check(tmp, H_lo, H_hi)) {
				Camera_Data = 0xF800;
				tmpcnt++;
//				batch[j]++;	//结果入桶
			}
...
uint8_t ImagDisp2(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height, uint16_t H)
{
	uint16_t cnt;
	uint16_t i, j; 
	uint16_t Camera_Data;
	...
	for(i = 0; i < height; i++)
	{
		for(j = 0; j < width; j++)
		{
			...
			tmp[1] = (Camera_Data & 0x07e0) >> 5;
			tmp[2] = (Camera_Data & 0x001f) << 1;
			if(i > 35 && i < 125 && color_check(tmp, H, H + 100)) {
				Camera_Data = 0xF800;
				tmpcnt++;
				batch[j]++;	//结果入桶
			}
...
/*
	...
	3：识别到，需右移
*/
void OV7725_to_ADC(uint8_t tmp) {/*
	if(tmp) {
